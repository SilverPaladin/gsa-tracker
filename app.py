...
# Rebuilding the full schema to ensure persistent task and chat data
conn = sqlite3.connect('gsa_command_v6.db', check_same_thread=False)
c = conn.cursor()
...
if not st.session_state.logged_in:
    ...
    with col:
        st.markdown("<h2 style='text-align:center; color:#5865f2; font-weight:900;'>GSA COMMAND</h2>", unsafe_allow_html=True)
        ...
        if st.button("AUTHORIZE"):
            ...
            if user:
                ...
                conn.commit(); st.success("Root Admin Initialized. Re-attempt login.")
    st.stop()
...
with st.sidebar:
    ...
    for mid, mname in c.execute("SELECT id, name FROM mods WHERE is_done=0 ORDER BY severity DESC").fetchall():
    ...
    for aid, aname in c.execute("SELECT id, name FROM mods WHERE is_done=1 ORDER BY id DESC").fetchall():
...
# 6.1 TRAINING ROSTER: 14-Day Timeline
if view == "CALENDAR":
    ...
    with cl:
        for i in range(12):
            ...
            st.markdown(f'<div style="background:#111214; padding:10px; border:1px solid #1e1e1e; margin-bottom:2px;">'
                        ...
                        f'<small style="color:#888;">{ev[0] if ev else "NO DATA"}</small></div>', unsafe_allow_html=True)
...
# 6.2 TASK VIEW & DISCUSSION THREAD
elif view == "MOD_VIEW":
    ...
    if mod:
        ...
        with col_r:
            ...
            for u, t, m in c.execute("SELECT user, timestamp, comment FROM comments WHERE mod_id=? ORDER BY id DESC", (mod[0],)).fetchall():
                st.markdown(f'<div class="thread-entry"><b>{u.upper()}</b> <small style="color:#5865f2">{t}</small><br>{m}</div>', unsafe_allow_html=True)
